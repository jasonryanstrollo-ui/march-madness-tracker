...
function GameCard({ g }) {
  ...
  return (
    <div style={{
      background: sa ? "rgba(239,68,68,.04)" : "rgba(255,255,255,.02)",
      border: sa ? "1px solid rgba(239,68,68,.2)" : "1px solid rgba(255,255,255,.05)",
      borderRadius: 12, padding: "12px 14px",
      animation: sa ? "glow 2.5s ease-in-out infinite, fadeIn .3s ease-out" : "fadeIn .3s ease-out",
      transition: "all .3s",
    }}>
      {/* Header */}
...
export default function Home() {
  ...
  const filtered = games
    ...
    .sort((a, b) => {
      const sa = isSpreadAlert(a) ? 0 : 1, sb = isSpreadAlert(b) ? 0 : 1;
      if (sa !== sb) return sa - sb;
      const ra = hasRun(a) ? 0 : 1, rb = hasRun(b) ? 0 : 1;
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "16px 12px env(safe-area-inset-bottom)" }}>
        ...
        {(sc > 0 || rc > 0) && (
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {sc > 0 && (
              <div style={{ flex: 1, background: "rgba(239,68,68,.06)", border: "1px solid rgba(239,68,68,.15)", borderRadius: 10, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {rc > 0 && (
              <div style={{ flex: 1, background: "rgba(245,158,11,.06)", border: "1px solid rgba(245,158,11,.15)", borderRadius: 10, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 16 }}>🔥</span>
                <div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: 40, color: "rgba(255,255,255,.2)", fontSize: 12 }}>
            {filter === "spread" ? "No spread alerts." : filter === "runs" ? "No scoring runs." : filter === "live" ? "No live games." : "No games today."}
          </div>
        ) : (
